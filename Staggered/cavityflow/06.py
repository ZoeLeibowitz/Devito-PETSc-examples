...
eq_p = Eq(p.laplace,
          rho * (1./dt * (ux_cc + vy_cc) - ux_cc**2 - 2*u.forward.dy*v.forward.dx - vy_cc**2),
...
# Velocity correction
update_u = Eq(u.forward, u.forward - (dt/rho)*(p.dx),
              subdomain=grid.subdomains['sub15'])

update_v = Eq(v.forward, v.forward - (dt/rho)*(p.dy),
