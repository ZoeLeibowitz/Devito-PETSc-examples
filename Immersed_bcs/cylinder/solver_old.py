...
def make_solver(nx, ny, ab2=False, implicit_diffusion=False):
    ...
    # Parabolic inflow profile for u at the west (inlet) boundary
    u_inflow = Function(name='u_inflow', grid=grid, space_order=so, staggered=y)
    u_max = 1.5
    y_extent = grid.extent[1]
    ...
    bc_u_tent = [EssentialBC(u.forward, u_inflow, subdomain=grid.subdomains['sub14'])]
    bc_u_tent += [Eq(u.forward, u.forward.subs({x: x - x.spacing}), subdomain=grid.subdomains['sub11'])]
    ...
    bc_v_tent += [Eq(v.forward, v.forward.subs({x+x.spacing/2: x-x.spacing/2}), subdomain=grid.subdomains['sub16'])]
    ...
    bc_u = [Eq(u.forward, u_inflow, subdomain=grid.subdomains['sub14'])]
    ...
    bc_v += [Eq(v.forward, v.forward.subs({x+x.spacing/2: x-x.spacing/2}), subdomain=grid.subdomains['sub16'])]
