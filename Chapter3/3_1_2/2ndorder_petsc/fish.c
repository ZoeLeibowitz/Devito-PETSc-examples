...
// for memory comparison with folder "2ndorder_petsc": run with
// mpiexec -n 1 ./fish -pc_type none -da_refine 3 -ksp_rtol 1e-12 -log_view 
// output says total memory (bytes) is 3.408e+05
...
//STARTCREATE
    ...
    PetscCall(SNESSetFunction(snes,NULL,Poisson2DFunctionGlobal,&user));

    PetscCall(DMCreateMatrix(da, &J));
    PetscCall(SNESSetJacobian(snes, J, J, Poisson2DJacobianGlobal, &user));
