...
PetscErrorCode Poisson2DFunctionGlobal(SNES snes, Vec X, Vec F, void *ctx)
{
    ...
    /* Create local ghosted vector */
    PetscCall(DMGetLocalVector(da, &Xlocal));
    PetscCall(DMGetLocalVector(da, &Flocal));

    /* Scatter from global X to local Xlocal (fill ghost cells) */
    PetscCall(DMGlobalToLocalBegin(da, X, INSERT_VALUES, Xlocal));
    PetscCall(DMGlobalToLocalEnd(da, X, INSERT_VALUES, Xlocal));

    /* Access arrays */
    PetscCall(DMDAVecGetArrayRead(da, Xlocal, &au));
    PetscCall(DMDAVecGetArray(da, Flocal, &aF));
    // PetscCall(DMDAVecGetArray(da, F, &aF));

    /* Get local grid info */
    PetscCall(DMDAGetLocalInfo(da, &info));

    /* Call original local function */
    PetscCall(Poisson2DFunctionLocal(&info, au, aF, user));

    /* Restore arrays */
    PetscCall(DMDAVecRestoreArrayRead(da, Xlocal, &au));
    PetscCall(DMDAVecRestoreArray(da, Flocal, &aF));
...
PetscErrorCode Poisson2DJacobianGlobal(SNES snes, Vec X, Mat J, Mat Jpre, void *ctx)
{
    PoissonCtx       *user = (PoissonCtx*)ctx;
    DM                da;
    Vec               Xlocal;
    ...
    /* Scatter global X → local Xlocal (ghost updates) */
    PetscCall(DMGlobalToLocalBegin(da, X, INSERT_VALUES, Xlocal));
    PetscCall(DMGlobalToLocalEnd(da, X, INSERT_VALUES, Xlocal));

    /* Access ghosted array */
    PetscCall(DMDAVecGetArrayRead(da, Xlocal, &au));

    /* Get local grid metadata */
    PetscCall(DMDAGetLocalInfo(da, &info));

    /* ---- Call your original local Jacobian builder ---- */
    PetscCall(Poisson2DJacobianLocal(&info, au, J, Jpre, user));

    /* Restore arrays */
    PetscCall(DMDAVecRestoreArrayRead(da, Xlocal, &au));

    /* Return ghosted local vector */
    PetscCall(DMRestoreLocalVector(da, &Xlocal));
    PetscFunctionReturn(PETSC_SUCCESS);
}
