    ...
    for (j = info->ys; j < info->ys + info->ym; j++) {
        ...
        for (i = info->xs; i < info->xs + info->xm; i++) {
            ...
            if (i==0 || i==info->mx-1 || j==0 || j==info->my-1 || i==1 || i==info->mx-2 || j==1 || j==info->my-2) {
                ...
                uee = ((i+2 == info->mx-1) || (i+2 == info->mx-2)) ? user->g_bdry(x+2*hx, y, 0.0, user) : au[j][i+2];

                uww = ((i-2 == 0) || (i-2 == 1))         ? user->g_bdry(x-2*hx, y, 0.0, user) : au[j][i-2];

                unn = ((j+2 == info->my-1) || (j+2 == info->my-2)) ? user->g_bdry(x, y+2*hy, 0.0, user) : au[j+2][i];

                uss = ((j-2 == 0) || (j-2 == 1))         ? user->g_bdry(x, y-2*hy, 0.0, user) : au[j-2][i];
