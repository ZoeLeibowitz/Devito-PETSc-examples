...
for n in n_values:
    ...
    # Gather full arrays on rank 0 before computing norms
    u_gathered = u.data._gather()
    v_gathered = v.data._gather()
    u_exact_gathered = u_exact.data._gather()
    lap_u_gathered = lap_u.data._gather()

    if comm is not None and configuration['mpi']:
        if comm != MPI.COMM_NULL and comm.rank == 0:
            u_exact_arr = np.asarray(u_exact_gathered)
            lap_u_arr = np.asarray(lap_u_gathered)
            u_arr = np.asarray(u_gathered)
            v_arr = np.asarray(v_gathered)

            u_diff_norm = np.linalg.norm((u_exact_arr - u_arr).ravel(), ord=np.inf)
            u_error = u_diff_norm / np.linalg.norm(u_exact_arr.ravel(), ord=np.inf)

            v_diff_norm = np.linalg.norm((lap_u_arr - v_arr).ravel(), ord=np.inf)
            v_error = v_diff_norm / np.linalg.norm(lap_u_arr.ravel(), ord=np.inf)
        else:
            u_error = None
            v_error = None
    else:
        # Serial fallback
        u_diff_norm = np.linalg.norm((u_exact.data[:] - u.data[:]).ravel(), ord=np.inf)
        u_error = u_diff_norm / np.linalg.norm(u_exact.data[:].ravel(), ord=np.inf)
        v_diff_norm = np.linalg.norm((lap_u.data[:] - v.data[:]).ravel(), ord=np.inf)
...
if comm.rank == 0:
    ...
    plt.ylabel(r"$\|\cdot\|_\infty$", fontsize=16)
    plt.title(f"Convergence of the infinity norm for $u$ and $v$ (MPI processes = {size})",
              fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig("3_3_1_convergence_mpi.png", dpi=300, transparent=True, metadata={})
    ...
    X_plot, Y_plot = np.meshgrid(tmpx, tmpy)

    plt.figure(figsize=(12, 10))

    plt.subplot(2, 2, 1)
    cp1 = plt.contourf(X_plot, Y_plot, np.asarray(u_gathered), levels=50, cmap='plasma')
    plt.colorbar(cp1)
    ...
    plt.subplot(2, 2, 3)
    cp2 = plt.contourf(X_plot, Y_plot, np.asarray(u_exact_gathered), levels=50, cmap='plasma')
    plt.colorbar(cp2)
    ...
    plt.subplot(2, 2, 2)
    cp3 = plt.contourf(X_plot, Y_plot, np.asarray(v_gathered), levels=50, cmap='plasma')
    plt.colorbar(cp3)
    ...
    plt.subplot(2, 2, 4)
    cp4 = plt.contourf(X_plot, Y_plot, np.asarray(lap_u_gathered), levels=50, cmap='plasma')
    plt.colorbar(cp4)
    ...
    plt.tight_layout()
    plt.savefig("3_3_1_solution_mpi.png", dpi=300, transparent=True, metadata={})
